		...
		if ($keyFile !== false && $dbFile !== false) {
			if (CreateDatabase($dbConnect,$package->databases) == true) {
				if ($dbConnect->select('site_table')->count() == 0) {
					$dbConnect->insert('site_table',array('domain'=>$_SERVER['HTTP_HOST'],'language'=>$language,'title'=>'iModule','description'=>'Site Description','templet'=>'default','templet_configs'=>'{}','logo'=>'{"default":-1,"footer":-1}','maskicon'=>'{"icon":-1,"color":"#0578bf"}','is_ssl'=>(isset($_SERVER['HTTPS']) == true ? 'TRUE' : 'FALSE'),'is_default'=>'TRUE','sort'=>0))->execute();
				} else {
