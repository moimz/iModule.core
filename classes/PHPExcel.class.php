...
class PHPExcelWriter {
	...
	function writePDF() {
		...
		$rendererLibraryPath = PHPEXCEL_ROOT.'/PHPExcel/lib/'.$rendererLibrary;
...
//		echo date('H:i:s') , " Write to PDF format using {$rendererName}" , EOL;
		...
		$this->writer->getActiveSheet()->setTitle('Orari');
		$objWriter = PHPExcel_IOFactory::createWriter($this->writer, 'PDF');
